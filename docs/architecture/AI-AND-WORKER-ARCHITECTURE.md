...
- **Prompt/template registry:** versioned templates per worker capability + task type; template version recorded on every call (with model version) for artifact attribution. **Implementation status (ACBP-P2-004, CDR-027):** BUILT as a provider-neutral, versioned, code-defined registry in `@acbp/contracts` (`model/template.ts`) — `TemplateDefinition {family, version, taskClass, segments, slots}`, `resolveTemplateRef` (deny-unknown), `latestTemplateRef` pinning, `templateProvenance {template_ref, template_version}` for artifact attribution (TASK-005), and `renderTemplateSegments` (own-slot substitution; assembly stays P2-007). CONFIG not tenant data — no DB/migration, no new SECURITY DEFINER. Final prompt wording + more families land with the consuming tickets (P2-005/P2-007/P2-008).
