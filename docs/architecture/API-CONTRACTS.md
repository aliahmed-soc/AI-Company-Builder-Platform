...
<!-- IMPLEMENTED at the ENGINE/use-case layer (ACBP-P2-008 generation, ACBP-P2-009 review + confirmation; CDR-029/030):
     the five review controls, the owner-only confirm (unlocks strategy), and the DISC-008 correction operate on the
     understanding-document VERSION with `expected_version` optimistic concurrency (owner-only `understanding:review`/
     `understanding:confirm`; member `understanding:read`). "Item decisions idempotent" + confirm/correct idempotency
     are realized by append-only event logs + `UNIQUE(document_id,kind)`. The HTTP request surface + live provider are
     the deferred owner gate (CDR-026 §0) — the engine is proven by the scripted/real-PG suites, exactly as P2-005/
     P2-008 deferred their routes. "staleness flags" beyond the strategy-unlock gate (cross-artifact dependents) is P3+. -->
