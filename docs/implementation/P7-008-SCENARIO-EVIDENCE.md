...
|---:|---|---|---|---|---|---|
| 1 | Model timeout | unmeasured | `return_value_only` | call → `timeout`; the deadline is enforced by the gateway, not the provider | FakeModelProvider `{ kind: "hang", ms }` — a real deadline, not a thrown error | callModel |
| 2 | Provider outage | unmeasured | `return_value_only` | fallback fires for an eligible class, and NEVER for a quality-bearing one | FakeModelProvider `{ kind: "fail", error: "provider_unavailable" }` on the primary | callModel |
| 3 | Invalid structured output | unmeasured | `return_value_only` | `invalid_output` after bounded re-asks — the cap is enforced, not advisory | FakeModelProvider scripted to return unparseable output on every call | callModel |
| 4 | Worker crash | unmeasured | `database_state` | run `running→failed(worker_lost)` after the heartbeat grace, read back from the database | a worker that stops heartbeating past the grace window, then the real reaper sweep | the run coordinator sweep |
...
| 8 | Tool/API failure (future external) | unmeasured | `database_state` | a throwing step is recorded as a failure with a category — but NOT a distinguishable one | a worker step that throws, driven through the real runtime | the worker runtime step executor |
...
| 12 | Partial completion (multi-step run) | unmeasured | `database_state` | resume from checkpoint — a killed run continues rather than restarting | a run killed mid-sequence, then resumed against the real checkpoint rows | the job step executor |
...
| 15 | Emergency stop during execution | unmeasured | `recorded_row` | the next tool call is blocked at the dispatcher and the refusal is RECORDED | a live stop activated between calls, then a real dispatch attempt | the worker runtime step boundary |
...
### 3. Invalid structured output — unmeasured
...
### 11. Duplicate delivery (job/event) — unmeasured
...
- **Mutation that should redden it:** Make `enqueueJob` skip its `findByIdempotencyKey` read-back, so a re-delivered key inserts a second job row.
- **Hosted CI run in which it did:** — *(not yet run)*
...
### 13. Usage-recording failure — unmeasured
