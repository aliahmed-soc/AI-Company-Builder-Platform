...
8. **Decision:** Primary = **`gpt-5.1`**, with the **exact dated snapshot read from the account model catalog and pinned in gateway configuration at ACBP-P2-003, before any production model call** (dateless alias never ships to production). Fallback = **`claude-sonnet-4-6`** (pinned-by-ID per Anthropic versioning semantics). _Note (per CDR-026 §0): P2-003 ships the gateway abstraction + a fake provider only and makes **no** production model call, so the catalog read + snapshot pin are DEFERRED to the live-provider owner gate — "at ACBP-P2-003" bounds when the pin is needed (before the first production call), not a claim that it was performed in this ticket._
