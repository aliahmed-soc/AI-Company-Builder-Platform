...
## ACBP-P1-011 implementation-time dispositions (Slice 6) - recorded 2026-07-23
- Query-plan decision (CDR-017 s10): NO index-only migration. Real-PG EXPLAIN evidence (portfolio-plan.integration.test.ts, realistic ~2,800-company/~2,300-membership ANALYZEd population on hosted postgres:16) shows the natural plan drives from company_memberships_member_idx with an indexed companies join, no sequential scan of either relation, bounded Limit + (created_at, id) Sort. Existing indexes adequate; migrations stay 0001-0009.
- Stale-drop semantics: a membership revoked between enumeration and enrichment DROPS the candidate (runInCompanyScope denies; never a stale/substituted row, never a fallback company); nextCursor derives from the last ENUMERATED candidate so pagination is unaffected. Direct implementation of CDR-017 "the next portfolio read excludes the revoked company" - not a new owner decision.
- Review findings dispositions (all Low/Info; register in docs/implementation/P1-011-REVIEW-COVERAGE.md): S-1 non-canonical base64url accepted (bound payload; threat model covers); S-2 up-to-101-tx page accepted (CDR-017 s3 isolation tradeoff); S-3 enrichment DB error -> 500 accepted (fail-closed; matches activity/company endpoints); L-1 epoch extraction exact on postgres:16 (numeric extract; proven at page-boundary microsecond tie); L-2 drop+hasMore composition proven by construction (nextCursor is a pure function of the enumerated page) + both halves integration-proven; L-3 param-allowlist-before-auth kept (generic 400 envelope, zero oracle, consistent with the activity route).

## ACBP-P1-012 workspace provisioning (CDR-018) - owner-accepted 2026-07-23
- Internal-PostgreSQL-only workspace provisioning; NO external provider/credentials/webhook/object-storage/hosting/codebase/db-service/sandbox (P0-005 canonically gates P5-011 only).
- Six canonical ordered steps: profile, mission_draft, research, roadmap, documents, activity. Auto-start after the creation tx commits; request-driven SEQUENTIAL execution; fresh CompanyScope tx per step; NO worker/queue/detached-task/polling/lease/daemon/outbox/owner-connection.
- Durable statuses pending|completed|failed; NO committed running state; one mutable row per (company, step); max 3 total attempts; exhausted -> safe conflict.
- Material effects: profile + activity VERIFY existing artifacts (no duplicates/synthetic events); mission_draft/research/roadmap/documents INSERT minimal company_workspace_areas registry rows (closed set, idempotent). No fake success (invariant 20).
- Activation: onboarding->active ONLY when all six completed; failed-and-acknowledged DEFERRED; provisioning.completed audited atomically with the transition.
...
- Implementation-level clarifications recorded during P1-013 (safer-reversible interpretations, no owner decision changed): (1) no reified AdminCapability VALUE exists anywhere - the capability is the freshly-verified position inside the single admin transaction (subsumes the branded-capability intent: nothing to cache, serialize, pass, or forge; boundary tests pin the absence of any generic scope primitive/impersonation identifier instead); (2) META_MAX_VALUE_LEN raised 512->1024 UTF-16 units so a 512-code-point astral verbatim reason survives the audit metadata bound (total-payload bound unchanged; boundary tests pin both); (3) malformed (non-UUID-shaped) selectors return the SAME coarse forbidden WITHOUT any database access (a uuid-cast 500 would have distinguished malformed from nonexistent); (4) all parse-failure causes on the admin route collapse to ONE generic 400 (stricter than the tenant surfaces' 413/415 split - no malformed-cause oracle on the admin surface).
