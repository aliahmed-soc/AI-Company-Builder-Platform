...
import { parseProfileUpdateBody, respondToProfileRequest } from '@/server/accounts/profile-http';
...
export async function GET(): Promise<Response> {
  return respondToProfileRequest(() => getAccountProfileForRequest());
}
...
export async function PATCH(request: Request): Promise<Response> {
  ...
  return respondToProfileRequest(() => updateAccountProfileForRequest(parsed.input));
}
