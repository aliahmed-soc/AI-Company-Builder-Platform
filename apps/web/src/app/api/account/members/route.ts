...
import { parseInviteBody, respondToMembersRequest } from '@/server/members/members-http';
...
export async function GET(): Promise<Response> {
  return respondToMembersRequest(() => listMembersForRequest());
}
...
export async function POST(request: Request): Promise<Response> {
  ...
  return respondToMembersRequest(() => inviteMemberForRequest(parsed.input));
}
