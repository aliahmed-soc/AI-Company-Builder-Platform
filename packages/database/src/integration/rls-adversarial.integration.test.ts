...
describe.skipIf(!hasTestDatabase)('RLS catalog audit + adversarial suite (real PostgreSQL) — ACBP-P1-006/CDR-013', () => {
  ...
  afterAll(async () => {
    ...
    if (admin) {
      ...
      for (const t of ['usage_events', 'understanding_confirmation_events', 'understanding_item_reviews', 'understanding_items', 'understanding_documents', 'memory_items', 'interview_answers', 'interview_questions', 'interview_sessions', 'platform_admins', 'provisioning_steps', 'company_workspace_areas', 'activity_events', 'company_memberships', 'company_profiles', 'companies', 'audit_events', 'memberships', 'account_profiles', 'accounts', 'identity_webhook_receipts', 'users']) await admin.kysely.schema.dropTable(t).ifExists().cascade().execute();
