...
async function cleanup(client: DatabaseClient): Promise<void> {
  ...
  for (const t of ['usage_events', 'planning_run_inputs', 'planning_runs', 'task_review_flags', 'credit_transactions', 'worker_runs', 'company_worker_states', 'worker_definitions', 'tool_definitions', 'job_checkpoints', 'jobs', 'tool_calls', 'task_runs', 'task_deletions', 'task_dependencies', 'tasks', 'milestones', 'goals', 'roadmaps', 'decisions', 'strategy_selections', 'strategy_recommendations', 'strategy_options', 'strategy_generations', 'understanding_confirmation_events', 'understanding_item_reviews', 'understanding_items', 'understanding_documents', 'memory_items', 'interview_answers', 'interview_questions', 'interview_sessions', 'platform_admins', 'provisioning_steps', 'company_workspace_areas', 'activity_events', 'company_memberships', 'company_profiles', 'companies', 'audit_events', 'memberships', 'account_profiles', 'accounts', 'identity_webhook_receipts', 'users', '_acbp_migration_probe', '_it_a', '_it_c', '_t_rollback', 'kysely_migration', 'kysely_migration_lock', '_it_migration', '_it_migration_lock']) {
    await drop(client, t);
  }
}
...
describe.skipIf(!hasTestDatabase)('database integration (real PostgreSQL)', () => {
  ...
  test('only the P1-002..P1-010 domain tables exist; later-ticket tables do not', async () => {
    ...
    // `understanding_item_reviews` + `understanding_confirmation_events` now exist (ACBP-P2-009, migration 0020).
    expect(names).toContain('understanding_item_reviews');
    expect(names).toContain('understanding_confirmation_events');
    // `tasks` + `task_dependencies` now exist (ACBP-P4-002, migration 0021).
    expect(names).toContain('tasks');
    expect(names).toContain('task_dependencies');
    // `strategy_generations` + `strategy_options` now exist (ACBP-P3-001, migration 0022).
    expect(names).toContain('strategy_generations');
