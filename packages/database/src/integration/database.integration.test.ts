...
describe.skipIf(!hasTestDatabase)('database integration (real PostgreSQL)', () => {
  ...
  test('only the P1-002..P1-010 domain tables exist; later-ticket tables do not', async () => {
    ...
    // `policies` + `policy_evaluations` now exist (ACBP-P6-001b, migration 0045).
    for (const policy of ['policies', 'policy_evaluations']) {
      expect(names).toContain(policy);
    }
    // Approval tables are still a later ticket (ACBP-P6-003) and must NOT exist yet.
