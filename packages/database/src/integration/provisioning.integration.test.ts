...
describe.skipIf(!hasTestDatabase)('workspace provisioning data model (real PostgreSQL, restricted role) — ACBP-P1-012/CDR-018', () => {
  ...
  test('DELETE and TRUNCATE are denied on both tables (no grant); areas UPDATE denied too', async () => {
    for (const t of ['usage_events', 'understanding_items', 'understanding_documents', 'memory_items', 'interview_answers', 'interview_questions', 'interview_sessions', 'platform_admins', 'provisioning_steps', 'company_workspace_areas'] as const) {
