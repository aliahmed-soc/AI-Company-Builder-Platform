// ACBP-P1-011 / CDR-017 â€” real-PostgreSQL proof of the membership-filtered company portfolio under the
...
//   - the portfolio contains ONLY companies where the actor holds an ACTIVE company_membership â€” account
...
describe.skipIf(!hasTestDatabase)('company portfolio (real PostgreSQL, restricted role) â€” ACBP-P1-011/CDR-017', () => {
  ...
  test('lists ONLY active-membership companies â€” account ownership, revoked, and other-account rows are excluded', async () => {
    ...
    // Account A owns this company, but userU is NOT a company member â†’ must be excluded (no account-owner registry view).
    await seedCompany(accountA, '2026-04-01T00:00:00.000000Z');
    // A revoked membership â†’ excluded.
    await seedCompany(accountA, '2026-05-01T00:00:00.000000Z', { userId: userU, role: 'owner', status: 'revoked' });
    // Another account's company (userV member) â†’ never appears for userU.
  ...
  test('the memberships self-branch is the real gate: a mismatched/absent app.current_actor returns nothing', async () => {
    ...
    // GUC is userU but the SQL asks for userV (another user) â†’ the WHERE excludes it too.
    ...
    // No app.current_actor at all â†’ self-branch cannot match â†’ nothing.
  ...
  test('keyset pagination: created_at DESC then id DESC, with an exact-microsecond tie broken by id', async () => {
    const coEarly = await seedCompany(accountA, '2026-01-01T00:00:00.000000Z', { userId: userU, role: 'owner' });
    // Two companies at the EXACT same microsecond instant â†’ the id DESC tie-break decides their order.
    ...
    // Page 1 (limit 2) then keyset-after its last row â†’ page 2 is strictly older (no overlap, no gap).
