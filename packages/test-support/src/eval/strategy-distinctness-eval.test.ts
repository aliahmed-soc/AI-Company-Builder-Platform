...
describe('ACBP-P3-006 — eval area 4, model-free half', () => {
  ...
  test('THE SCORER ITSELF refuses a malformed dataset — the guard is not bypassable through opts.cases', () => {
    /*
     * The guard's docstring says it throws "so a malformed dataset can never be scored at all". That was true of
     * `pnpm eval:area-4`, which calls it before scoring, and FALSE of `scoreStrategyDistinctness`, which is how
     * the CI suite scores and which takes a caller-supplied dataset. One argument bypassed it.
     *
     * This asserts the sentence is now true of the scoring path too. Without the call added in this change, the
     * line below returns a report — a 100% rejection verdict over a case that planted nothing to reject.
     */
    const vacuous: DistinctnessEvalCase = { ...STRATEGY_DISTINCTNESS_CASES[0]!, id: 'vacuous-via-scorer', seededDuplicates: 1 };
    expect(() => scoreStrategyDistinctness({ cases: [vacuous] })).toThrow(
      /case vacuous-via-scorer declares 1 seeded near-duplicate\(s\) but contains 0/,
    );
  });

  test('a WELL-FORMED dataset still scores — the new guard refuses nothing it should accept', () => {
    // The other half of the guard. A check that threw on everything would satisfy the test above and destroy
    // the command; this pins that the real dataset still produces a report.
    expect(() => scoreStrategyDistinctness()).not.toThrow();
    const report = scoreStrategyDistinctness();
    expect(report.cases.length).toBe(STRATEGY_DISTINCTNESS_CASES.length);
    expect(report.caseCount).toBe(STRATEGY_DISTINCTNESS_CASES.length);
  });
