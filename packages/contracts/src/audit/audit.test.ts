...
describe('task.failed schema version 2 — retry_state (ACBP-P5-013; the deferral P5-002 named)', () => {
  test('carries retry_state, and the registry says version 2', () => {
    ...
    const ev = taskFailed({ taskId: 't1', runId: 'r1', attempt: 2, failureCategory: 'provider_error', retryState: 'scheduled' });
    expect(ev.metadata).toMatchObject({ run_id: 'r1', attempt: 2, failure_category: 'provider_error', retry_state: 'scheduled' });
