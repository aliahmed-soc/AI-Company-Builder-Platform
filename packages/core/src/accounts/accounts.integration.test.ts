...
describe.skipIf(!hasTestDatabase)('account provisioning + profile (real PostgreSQL, restricted role) — ACBP-P1-003/006', () => {
  ...
  afterAll(async () => {
    ...
    if (seed) {
      ...
      for (const t of ['platform_admins', 'provisioning_steps', 'company_workspace_areas', 'activity_events', 'company_memberships', 'company_profiles', 'companies', 'audit_events', 'memberships', 'account_profiles', 'accounts', 'identity_webhook_receipts', 'users']) await seed.kysely.schema.dropTable(t).ifExists().cascade().execute();
