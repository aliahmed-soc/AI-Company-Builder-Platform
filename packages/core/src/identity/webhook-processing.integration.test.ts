...
describe.skipIf(!hasTestDatabase)('identity webhook processor (real PostgreSQL)', () => {
  ...
  beforeAll(async () => {
    ...
    for (const t of ['identity_webhook_receipts', 'users', 'kysely_migration', 'kysely_migration_lock']) {
  ...
  afterAll(async () => {
    if (client) {
      for (const t of ['identity_webhook_receipts', 'users']) await client.kysely.schema.dropTable(t).ifExists().cascade().execute();
