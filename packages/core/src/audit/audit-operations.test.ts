...
describe('audit completeness registry (ACBP-P1-008 / CDR-014)', () => {
  test('the approved operation set is exactly the two membership lifecycle operations', () => {
    expect([...AUDITED_OPERATION_IDS].sort()).toEqual(['membership.invite', 'membership.revoke']);
  ...
  test('factoryFor produces an event whose name matches the operation mapping', () => {
    for (const op of AUDITED_OPERATION_IDS) {
      const event = factoryFor(op)('subject_1', 'viewer');
