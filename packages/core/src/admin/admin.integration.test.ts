// ACBP-P1-013 / CDR-019 â€” real-PostgreSQL proof of the platform-admin tenant read. Trust-critical: the reason
...
describe.skipIf(!hasTestDatabase)('platform-admin tenant read (real PostgreSQL, restricted role) â€” ACBP-P1-013/CDR-019', () => {
  ...
  beforeEach(async () => {
    ...
    // Operational (owner-connection) admin setup â€” the runbook path.
  ...
  test('happy path: active admin + valid reason + matching pair â†’ four-field DTO + exactly one TARGET-tenant admin audit', async () => {
  ...
  test('the reason is retained VERBATIM (leading/trailing whitespace + astral content preserved exactly)', async () => {
    const raw = '  \u{1F50D} inspect Ð¿Ð¾ÑÐ»Ðµ report  ';
  ...
  test('reason gate runs FIRST: invalid reasons return invalid_reason (even for non-admins) and write NOTHING', async () => {
    for (const bad of [undefined, '', '   ', 'x'.repeat(513), 42]) {
      ...
      // A NON-admin with a bad reason ALSO gets invalid_reason â€” proving the validator precedes the admin lookup.
  ...
  test('authority: tenant roles NEVER grant admin â€” owner/viewer/ordinary/revoked-admin all get ONE coarse forbidden, zero writes', async () => {
  ...
  test('target isolation: mismatched pair / unknown ids get the SAME coarse forbidden (no existence oracle); the right pair still works per-request', async () => {
    // Company B addressed under account A â†’ relationship check fails â†’ forbidden.
    ...
    // Unknown company / unknown account â†’ the IDENTICAL result shape.
    ...
    // MALFORMED ids (non-UUID shape, SQL-ish junk, empty) â†’ the SAME coarse forbidden â€” never a thrown
  ...
  // The failpoint fires AFTER the audit write, inside the same transaction â€” a rejection there proves the
  // rollback discards BOTH the audit row and the data path (the no-argument failpoint replaced the removed
  // auditWriter seam, which handed the live target-scoped transaction to injectable code â€” review M finding).
  ...
  test('a SOFT-DELETED admin is denied by the DATABASE-LAYER gate (users join) â€” the same coarse forbidden', async () => {
  ...
  // NOTE: the no-impersonation SOURCE guard lives in admin-boundary.test.ts (always runs â€” no database needed).
});
