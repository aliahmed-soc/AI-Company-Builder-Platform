...
export const TRUST_CRITICAL_INDEX = Object.freeze([
  ...
  },
  {
    ...
    builtBy: 'ACBP-P0-017 (logs half); the AUDIT half is built by nobody',
    status: 'unmeasured',
    anchor: 'recorded_row',
    file: 'packages/observability/src/logger.test.ts',
    testTitle: 'sentinel secret never appears in emitted logs (metadata + error)',
    entryPoint: 'logger',
    mutation: 'Remove redact() from the metadata path and log a planted sentinel.',
    mutationRunId: '',
    doesNotProve:
      'THE AUDIT HALF, which nothing protects. `boundedMetadata` rejects objects, arrays, Errors and null and then accepts ANY string up to 1024 units with no secret detection — safety rests on typed factories happening to carry scalars, which is a convention, not a control. The test title honestly names its own scope: metadata + error. ACBP-P7-007 slice 4 builds the audit sweep.',
  },
  {
    number: 17,
